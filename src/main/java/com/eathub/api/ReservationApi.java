...
public class ReservationApi {
    ...
    @GetMapping("/get/reservations/admin")
    public ResponseEntity<?> getRequestReservation(@RequestParam("page") int page, HttpSession session){
        Long member_seq = (Long) session.getAttribute(SessionConf.LOGIN_MEMBER_SEQ);

        List<ReservationDTO> reservationList = memberService.getRequestReservations((page-1)*2,member_seq);
        List<RestaurantDetailDTO> restaurantDetailList = restaurantService.getRestaurantDetailList();
        ...
        if(reservationList != null){
            for (ReservationDTO reservationDTO : reservationList) {
                ...
                for (RestaurantDetailDTO restaurantDetailDTO : restaurantDetailList) {
                    if (restaurant_seq.equals(restaurantDetailDTO.getRestaurant_seq())) {
                        reservationDTO.setImage_url(restaurantDetailDTO.getImage_url());
                        break;
                    }
                }
    ...
    @PostMapping("/post/reservations/reject")
    public ResponseEntity<?> reservationReject(@RequestParam("res_seq") Long res_seq, HttpSession session){
        ...
        // res_seq 가지고 reservation 테이블의 res_status => REJECT로 변경
        // 성공시 SUCCESS , 실패시 FAIL 값 반환
        String result = memberService.rejectReservation(res_seq, mem_seq);

        if ("SUCCESS".equals(result)) {
            // 성공 응답
            return ResponseEntity.ok().body(Map.of("message", "예약을 거절했습니다."));
        }

        return ResponseEntity.status(HttpStatus.PRECONDITION_FAILED).body(Map.of("message", "예약거절에 실패했습니다."));

    }

    @PostMapping("/post/reservations/access")
    public ResponseEntity<?> reservationAccess(@RequestParam("res_seq") Long res_seq, HttpSession session){
        ...
        // res_seq 가지고 reservation 테이블의 res_status => REJECT로 변경
        // 성공시 SUCCESS , 실패시 FAIL 값 반환
        String result = memberService.accessReservation(res_seq, mem_seq);

        if ("SUCCESS".equals(result)) {
            // 성공 응답
            return ResponseEntity.ok().body(Map.of("message", "예약을 승인했습니다."));
        }

        return ResponseEntity.status(HttpStatus.PRECONDITION_FAILED).body(Map.of("message", "예약승인에 실패했습니다."));
